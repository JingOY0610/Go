...
class GO:
    def __init__(self, n):
        ...
        self.max_move = int(math.floor(1.5 * n * n) // 2 * 2) # The max movement of a Go game
        self.komi = n/4 # Komi rule
    ...
    def place_chess(self, i, j, piece_type, previous_died_pieces):
        ...
        valid_place = self.valid_place_check(i, j, piece_type, previous_died_pieces)
        if not valid_place:
            return False
    ...
    def valid_place_check(self, i, j, piece_type, previous_died_pieces, test_check=False):
        '''
        ...
        # Check special case: repeat placement of died piece (KO rule)
            ...
            if previous_died_pieces and (i,j) in previous_died_pieces:
                if verbose:
    ...
    def game_end(self, piece_type):
        ...
        for i in range(self.size):
            for j in range(self.size):
                if self.valid_place_check(i, j, piece_type, self.died_pieces, test_check=True):
                    return False
        return True
    ...
    def play(self, player1, player2, verbose=False):
        ...
        while 1:
            ...
            # Game continues
            if piece_type == 1: i, j = player1.get_input(self, piece_type, self.died_pieces)
            else: i, j = player2.get_input(self, piece_type, self.died_pieces)

            # If invalid input, continue the loop. Else it places a chess on the board.
            if not self.place_chess(i, j, piece_type, self.died_pieces):
                if verbose:
                    self.visualize_board() 
                continue
